...
function inicializarElementos() {
    ...
    elementos.filterButtons = Array.from(document.querySelectorAll(".filtros-bar .btn"));
...
function configurarEventos() {
    ...
    if (elementos.listContainer) {
        elementos.listContainer.addEventListener("click", (event) => {
            ...
            if (!card || !elementos.listContainer.contains(card) || !card.dataset.pacienteId) {
                return;
            }
...
async function carregarPacientes() {
    ...
    try {
        ...
        const payload =
            (resposta?.dados?.pacientes?.data && Array.isArray(resposta.dados.pacientes.data)
            ...
            (resposta?.pacientes && Array.isArray(resposta.pacientes)
                ? resposta.pacientes
                : null) ||
            (resposta?.data && Array.isArray(resposta.data)
                ? resposta.data
                : null) ||
...
function filtrarPacientes() {
    return state.pacientes.filter((paciente) => {
        if (state.filtroStatus === "com-psicologo" && !paciente.comPsicologo) return false;
        if (state.filtroStatus === "sem-psicologo" && paciente.comPsicologo) return false;

        if (state.busca) {
            const texto = `${paciente.nome} ${paciente.cpf} ${paciente.email}`.toLowerCase();
...
function criarCardPaciente(paciente, index) {
    ...
    spanStatus.textContent = paciente.comPsicologo ? "Com Psicólogo" : "Sem Psicólogo";
...
function abrirModal(paciente) {
    ...
    if (elementos.modalCpf) elementos.modalCpf.textContent = `CPF: ${paciente.cpf}`;
    if (elementos.modalNasc) elementos.modalNasc.textContent = `Nascimento: ${paciente.nascimento}`;
    if (elementos.modalEmail) elementos.modalEmail.textContent = paciente.email;
    if (elementos.modalTelefone) elementos.modalTelefone.textContent = paciente.telefone;
    if (elementos.modalPsicologo) elementos.modalPsicologo.textContent = paciente.psicologo;
    if (elementos.modalUltimaSessao) elementos.modalUltimaSessao.textContent = paciente.ultimaSessao;
